...
class InferencePipeline(torch.nn.Module):
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache1.append({
                "k": torch.zeros([batch_size, 30*44*80//4, 24, 128], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, 30*44*80//4, 24, 128], dtype=dtype, device=device)
            })
    ...
    def inference(self, noise: torch.Tensor, text_prompts: List[str], start_latents: Optional[torch.Tensor] = None, return_latents: bool = False) -> torch.Tensor:
        ...
        for block_index in range(num_blocks):
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                timestep = torch.ones(
                    [batch_size, self.num_frame_per_block], device=noise.device, dtype=torch.long) * current_timestep
