from causvid.models.wan22.wan_wrapper import Wan22DiffusionWrapper, Wan22TextEncoder, Wan22VAEWrapper
...
def init_model(device):
    model = Wan22DiffusionWrapper().to(device).to(DTYPE)
    encoder = Wan22TextEncoder().to(device).to(DTYPE)
...
def main():
    ...
    for index in tqdm(range(int(math.ceil(len(dataset) / dist.get_world_size()))), disable=dist.get_rank() != 0):
        ...
        latents = torch.randn(
            [1, 31, 48, 44, 80], dtype=DTYPE, device=device
        )
        ...
        for progress_id, t in enumerate(tqdm(scheduler.timesteps)):
            timestep = t * \
                torch.ones([1, 31], device=device, dtype=DTYPE)
            ...
            latents = scheduler.step(
                ...
                scheduler.timesteps[progress_id] * torch.ones(
                    [1, 31], device=device, dtype=torch.long).flatten(0, 1),
                latents.flatten(0, 1)
        ...
        noisy_inputs = noisy_inputs[:, [0, 36, 44, 48, -1]]
